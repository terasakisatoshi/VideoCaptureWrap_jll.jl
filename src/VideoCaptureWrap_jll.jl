...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import VideoCaptureWrap_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function find_artifact_dir()
        return artifact"VideoCaptureWrap"
    end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("VideoCaptureWrap_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "VideoCaptureWrap_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"VideoCaptureWrap", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "VideoCaptureWrap_jll", "src", "VideoCaptureWrap_jll.jl"))
        @info("VideoCaptureWrap_ll dev'ed out to /home/terasaki/.julia/dev/VideoCaptureWrap_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("b69a920f-0f27-5c26-a11f-67bef8892cc7"))
platforms = [Pkg.Artifacts.unpack_platform(e, "VideoCaptureWrap", artifacts_toml) for e in artifacts["VideoCaptureWrap"]]
...
if best_platform === nothing
    @debug("Unable to load VideoCaptureWrap; unsupported platform $(triplet(platform_key_abi()))")
else
